    ...
    public static int Main(string[] args)
    {
        try
        {
            ...
            switch (args[0])
            {
                ...
                case "preflight-snas-registration": FactoryProfilePreflight.WriteSnas(
                    Many(options, "--mod"), One(options, "--output"), One(options, "--repo-root"),
                    One(options, "--profile"), One(options, "--upstream"), One(options, "--closure"),
                    One(options, "--assembly"), One(options, "--authored-profiles"),
                    One(options, "--canonical-managed-root"), One(options, "--dotnet"), One(options, "--content-plan")); break;
                ...
                case "inspect-compiled-factories":
                    using (var contract = SelectedFactoryContract.Load(One(options, "--manifest"), One(options, "--authored-profiles")))
                    {
                        var inspected = CompiledFactoryInspection.Inspect(One(options, "--assembly"), contract.Profiles.RootElement, contract.Factories);
                        File.WriteAllText(One(options, "--output"), JsonSerializer.Serialize(inspected, new JsonSerializerOptions { WriteIndented = true }) + "\n");
                    }
                    break;
                case "inspect-legacy-regression-entries":
                    File.WriteAllText(One(options, "--output"), JsonSerializer.Serialize(
                        CompiledFactoryInspection.InspectLegacyRegressionEntries(One(options, "--assembly")),
