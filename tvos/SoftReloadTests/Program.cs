...
SoftReloadStateMachine Ready()
{
    SoftReloadStateMachine value = new();
    Require(value.TryRequest(true), "request rejected");
    return value;
}
void ReachVerifying(SoftReloadStateMachine value)
{
...
{
    SoftReloadStateMachine value = Ready();
...
Test("unverified mutation does not request reload", () =>
    Require(!new SoftReloadStateMachine().TryRequest(false), "unverified request accepted"));
Test("preparation ticket generation mismatch fails blocked", () =>
{
    SoftReloadStateMachine value = Ready(); value.Fail("reload-generation-mismatch");
...
{
    SoftReloadStateMachine value = Ready(); value.Fail("reload-materialization-mismatch");
    Require(value.State == TvOSSoftReloadPhase.Failure, "mismatch resumed");
});
Test("Settings reload failure preserves stale guard state", () =>
{
    SoftReloadStateMachine value = Ready(); value.EnterSettings(); value.Fail("settings-reload-failed");
...
{
    SoftReloadStateMachine value = Ready(); value.EnterSettings(); value.Fail("input-reload-failed");
    Require(value.State == TvOSSoftReloadPhase.Failure, "input failure resumed");
});
Test("main-menu timeout preserves stale guard state", () =>
{
    SoftReloadStateMachine value = Ready(); value.EnterSettings(); value.EnterGameState(); value.WaitForMainMenu(); value.Fail("main-menu-timeout");
...
{
    SoftReloadStateMachine value = Ready(); ReachVerifying(value); value.Fail("reload-completion-ticket-mismatch");
    Require(value.State == TvOSSoftReloadPhase.Failure, "completion mismatch resumed");
});
Test("failure cannot blindly retry", () =>
{
    SoftReloadStateMachine value = Ready(); value.Fail("partial-reload");
    Require(!value.TryRequest(true), "unsafe retry accepted");
});
Test("success allows a later independent mutation", () =>
{
    SoftReloadStateMachine value = Ready(); ReachVerifying(value); value.Complete();
...
{
    SoftReloadStateMachine value = Ready(); value.Fail("failed");
