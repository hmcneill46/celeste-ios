...
internal sealed class SaveManagerService : IDisposable
{
    ...
    private readonly PersistenceStore persistence;
    private readonly List<NSObject> observers = new();
    private readonly Dictionary<NWConnection, ConnectionState> connections = new();
    private readonly SaveManagerConnectionGate connectionGate = new(SaveManagerHttpProtocol.MaximumConcurrentConnections);
    ...
    private SaveManagerHttpProtocol? protocol;
    ...
    internal SaveManagerService(PersistenceStore store)
    {
        ...
        RuntimeLog.Info("STAGE10A_DORMANT listener=false; bonjour=false; activation=explicit-options-only");
#if TVOS_SAVE_MANAGER_AUTOMATION
        RuntimeLog.Warning("STAGE10A_AUTOMATION enabled=true; evidence=ignored-local-only");
        _ = Task.Run(async () =>
        {
    ...
    internal void CompleteSoftReload()
    {
        lock (gate)
        {
            ...
            RuntimeLog.Info("STAGE13B_MANAGER result=complete; restart-required=false; listener=false; sessions=cleared");
        }
    }
    ...
    internal void MarkSoftReloadFailed(string category)
    {
        lock (gate)
        {
            ...
            RuntimeLog.Warning($"STAGE13B_MANAGER result=reload-failed; category={SanitizeReason(category)}; restart-required=true");
        }
    }
    ...
    private async Task StartCoreAsync()
    {
        try
        {
            ...
            SaveExportSnapshot stableSnapshot = persistence.CreateReadOnlySaveManagerSnapshot();
            ...
            if (addresses.Count == 0)
            {
                ...
                RuntimeLog.Warning("STAGE10A_START result=no-lan-address; listener=false; bonjour=false");
                return;
            }

            SaveManagerHttpProtocol nextProtocol = new(
                stableSnapshot,
        ...
        catch (Exception exception)
        {
            ...
            RuntimeLog.Error($"STAGE10A_START result=failed; type={exception.GetType().Name}; message={exception.Message}");
            StopNetworkObjects(clearStatus: false);
        }
    }
        ...
        lock (gate)
        {
            ...
            switch (listenerState)
            {
                case NWListenerState.Ready:
                    ...
                    string[] urls = addresses.Select(address => SaveManagerLanAddressPolicy.FormatUrl(address, port)).ToArray();
                    TvOSSaveManagerQrImage? pairingQr = null;
                    string pairingStatus = "unavailable";
                    try
                    {
                        SaveManagerHttpProtocol currentProtocol = protocol
                        ...
                        pairingQr = SaveManagerQrCodeGenerator.Create(pairingUrl);
                        pairingStatus = "available";
                        RuntimeLog.Info($"STAGE15_QR result=ready; pixels={pairingQr.Width}; modules={pairingQr.ModuleCount}; integer-scale={pairingQr.IntegerScale}; correction={SaveManagerQrCodeGenerator.CorrectionLevel}; quiet-zone={SaveManagerQrCodeGenerator.QuietZoneModules}; credential=not-logged");
                    }
                    catch (Exception exception)
                    {
                        RuntimeLog.Warning($"STAGE15_QR result=unavailable; type={exception.GetType().Name}; credential=not-logged; manual-fallback=true");
                    }
                    status = new TvOSSaveManagerDisplayState
                    {
                        Phase = "ready",
                    ...
                    RuntimeLog.Warning($"STAGE10A_AUTOMATION_READY url={urls[0]}; access-code={protocol?.AccessCode}");
#endif
                    break;
                ...
                case NWListenerState.Cancelled:
                    RuntimeLog.Info("STAGE10A_LISTENER state=cancelled; url=cleared");
                    break;
            }
        }
    }
    ...
    private void AcceptConnection(NWListener source, NWConnection connection)
    {
        lock (gate)
        {
            ...
            connection.SetStateChangeHandler((value, error) =>
            {
                ...
                {
                    if (error != null) RuntimeLog.Warning($"STAGE10A_CONNECTION state={value}; error={SafeError(error)}");
                    CloseConnection(connection, $"state-{value.ToString().ToLowerInvariant()}");
                }
            });
            connection.Start();
            RuntimeLog.Info($"STAGE10A_CONNECTION result=accepted; active={connectionGate.Count}");
            Receive(connection);
        }
    }
            ...
            (data, size, _, complete, error) =>
            {
                try
                {
                    lock (gate)
                    {
                        ...
                        SaveManagerRequestProgress progress = SaveManagerHttpProtocol.InspectRequestProgress(buffered);
                        ...
                        if (!progress.Complete && !complete)
                        {
                            int ceiling = progress.ExpectedBytes > 0
                                ...
                                : SaveManagerHttpProtocol.MaximumHeaderBytes;
                        ...
                        RuntimeLog.Info($"STAGE10A_HTTP status={response.StatusCode}; request-bytes={buffered.Length}; response-bytes={response.Body.Length}; authenticated-sessions={current.ActiveSessionCount}{authEvidence}{payloadEvidence}");
                        SendResponse(connection, response, headOnly);
                    }
                }
                catch (Exception exception)
                {
                    RuntimeLog.Error($"STAGE10A_CONNECTION result=failed; type={exception.GetType().Name}; message={exception.Message}");
                    CloseConnection(connection, "managed-exception");
                }
            });
    }

    private void SendResponse(NWConnection connection, SaveManagerHttpResponse response, bool headOnly)
    {
        if (!connections.TryGetValue(connection, out ConnectionState? state)) return;
        ...
        state.Timer.Change(SaveManagerConnectionPolicy.ResponseCloseLifetime, Timeout.InfiniteTimeSpan);
        byte[] encoded = response.Encode(headOnly);
        connection.Send(encoded, NWContentContext.FinalMessage, true, error =>
        {
            if (error != null) RuntimeLog.Warning($"STAGE10A_CONNECTION result=send-error; error={SafeError(error)}");
    ...
    private void CloseConnection(NWConnection connection, string reason)
    {
        lock (gate)
        {
            ...
            RuntimeLog.Info($"STAGE10A_CONNECTION result=closed; reason={SanitizeReason(reason)}; active={connectionGate.Count}");
    ...
    private void ConnectionTimedOut(NWConnection connection)
    {
        ...
        RuntimeLog.Warning($"STAGE10A_CONNECTION result=timeout; phase={reason}");
    ...
    private TvOSSaveManagerDisplayState Status()
    {
        lock (gate)
        {
            ...
            {
                SaveManagerPairingState pairing = protocol.PairingState;
                string next = pairing switch
                {
                    SaveManagerPairingState.Available => "available",
                    SaveManagerPairingState.Consumed => "connected",
                    SaveManagerPairingState.Expired => "expired",
                    _ => "unavailable"
                };
                if (next != status.PairingStatus)
                    status = CopyStatus(status, pairingStatus: next,
                        pairingQr: pairing == SaveManagerPairingState.Available ? status.PairingQr : null,
                        preservePairingQr: pairing == SaveManagerPairingState.Available);
            }
    ...
    private void Stop(string reason)
    {
        lock (gate)
        {
            ...
            if (wasActive) RuntimeLog.Info($"STAGE10A_STOP reason={SanitizeReason(reason)}; listener=false; sessions=cleared; url=cleared; access-code=cleared");
        }
    }
    ...
    private void SetIdleTimerSuppressed(bool suppressed)
    {
        ...
        RuntimeLog.Info($"STAGE10A_IDLE_TIMER suppressed={suppressed.ToString().ToLowerInvariant()}");
    }
    ...
    internal static IReadOnlyList<string> DiscoverLanAddresses()
    {
        ...
        try
        {
            ...
            for (IntPtr current = head; current != IntPtr.Zero && visited.Add(current) && visited.Count <= 256;)
            {
                ...
                if (string.IsNullOrWhiteSpace(name) || SaveManagerLanAddressPolicy.IsExcludedInterface(name)) continue;
                IPAddress? address = SaveManagerLanAddressPolicy.ReadDarwinSockAddr(item.Address);
                if (address == null) continue;
                int rank = SaveManagerLanAddressPolicy.InterfaceRank(name);
                if (address.AddressFamily == AddressFamily.InterNetwork && SaveManagerLanAddressPolicy.IsUsableIpv4(address)) values.Add((rank, address.ToString()));
                else if (address.AddressFamily == AddressFamily.InterNetworkV6 && SaveManagerLanAddressPolicy.IsUsableIpv6(address)) values.Add((rank + 10, address.ToString()));
            }
        }
    ...
    private sealed class ConnectionState : IDisposable
    {
        ...
        internal int NextReceiveMaximum { get; set; } = SaveManagerHttpProtocol.MaximumHeaderBytes;
