...
internal sealed class Stage6PersistenceLifecycle : IDisposable
{
    private readonly List<NSObject> observers = new();
    private bool disposed;

    internal Stage6PersistenceLifecycle(Stage6PersistenceStore persistenceStore)
    {
    ...
    private void Observe(NSString notification, string reason)
    {
        observers.Add(NSNotificationCenter.DefaultCenter.AddObserver(notification, _ =>
        {
            ...
            Stage3BLog.Info($"STAGE6_LIFECYCLE_FLUSH reason={reason}; result={(result ? "completed-or-unchanged" : "failed")}");
        }));
    }
    ...
    public void Dispose()
    {
        ...
        Stage3BLog.Info($"STAGE6_LIFECYCLE_FLUSH reason=disposal; result={(result ? "completed-or-unchanged" : "failed")}");
