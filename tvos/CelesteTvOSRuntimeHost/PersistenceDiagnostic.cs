...
internal static class PersistenceDiagnostic
{
    private static int passed;

    internal static void Run(PersistenceStore store, string sessionRoot)
    {
        ...
        RuntimeLog.Info($"STAGE9B_DIAGNOSTIC_PASS tests={passed}; namespace=tests; bridge-bytes={store.BridgeBytes()}");
        RuntimeLog.Info($"STAGE6_DIAGNOSTIC_PASS tests={passed}; namespace=tests; bridge-bytes={store.BridgeBytes()}");
    }

    private static void BasicV2(
        PersistenceStore store,
        IReadOnlyList<PersistenceStore.DiagnosticEntry> empty,
        IReadOnlyList<PersistenceStore.DiagnosticEntry> one,
        IReadOnlyList<PersistenceStore.DiagnosticEntry> two,
        IReadOnlyList<PersistenceStore.DiagnosticEntry> all
    )
    {
        ...
        byte[] oneEncoded = PersistenceStore.EncodeForDiagnostics(1, one);
        PersistenceStore.DiagnosticDecoded oneDecoded = PersistenceStore.DecodeForDiagnostics(oneEncoded);
        ...
        byte[] twoEncoded = PersistenceStore.EncodeForDiagnostics(2, two);
        store.WriteRawForDiagnostics("B", twoEncoded);
        Assert(store.Restore().Generation == 2, "two-valid-v2-newest-wins");

        store.WriteRawForDiagnostics("A", PersistenceStore.EncodeForDiagnostics(5, one));
        store.WriteRawForDiagnostics("B", PersistenceStore.EncodeForDiagnostics(4, two));
        Assert(store.Restore().Generation == 5, "highest-generation-selected-not-format-order");

        byte[] deleted = PersistenceStore.EncodeForDiagnostics(6, empty);
        PersistenceStore.DiagnosticDecoded deletedDecoded = PersistenceStore.DecodeForDiagnostics(deleted);
        Assert(deletedDecoded.Entries.All(entry => !entry.Present && entry.StoredBytes == 0), "deleted-entries-have-no-payload");

        byte[] allEncoded = PersistenceStore.EncodeForDiagnostics(7, all);
        PersistenceStore.DiagnosticDecoded allDecoded = PersistenceStore.DecodeForDiagnostics(allEncoded);
        Assert(allDecoded.Entries.Count(entry => entry.Present) == 4, "settings-and-three-save-slots");
        Assert(allDecoded.EnvelopeBytes < PersistenceStore.HardEnvelopeBudgetBytes, "basic-all-slot-envelope-budget");
    }

    private static void CompressionAndCorruption(IReadOnlyList<PersistenceStore.DiagnosticEntry> entries)
    {
        byte[] source = Encoding.UTF8.GetBytes(string.Concat(Enumerable.Repeat("Celeste-v2-deterministic\n", 4096)));
        byte[] compressedA = PersistenceStore.CompressForDiagnostics(source);
        byte[] compressedB = PersistenceStore.CompressForDiagnostics(source);
        Assert(compressedA.SequenceEqual(compressedB), "deterministic-zlib-level9");
        Assert(PersistenceStore.DecompressForDiagnostics(compressedA, source.Length, source.Length).SequenceEqual(source),
            "bounded-compression-roundtrip");

        byte[] encoded = PersistenceStore.EncodeForDiagnostics(20, entries);
        ...
        BinaryPrimitives.WriteUInt32LittleEndian(
            ...
            checked((uint)(PersistenceStore.HardCompressedEntryBudgetBytes + 1))
        );
        ...
        BinaryPrimitives.WriteUInt32LittleEndian(
            ...
            checked((uint)(PersistenceStore.MaximumSaveDataBytes + 1))
        );
        ...
        byte[] incompressibleStored = PersistenceStore.CompressForDiagnostics(incompressible);
        Assert(incompressible.Length <= PersistenceStore.MaximumSaveDataBytes, "incompressible-valid-fixture-uncompressed-bounded");
        Assert(incompressibleStored.Length <= PersistenceStore.HardCompressedEntryBudgetBytes, "incompressible-valid-fixture-stored-bounded");
        Assert(PersistenceStore.DecompressForDiagnostics(incompressibleStored, incompressible.Length,
            PersistenceStore.MaximumSaveDataBytes).SequenceEqual(incompressible), "incompressible-valid-fixture-roundtrip");
    }
    ...
    {
        store.ClearNamespaceForDiagnostics();
        byte[] v1A = PersistenceStore.EncodeForDiagnostics(73, one, PersistenceStore.PreviousFormatVersion);
        byte[] v1B = PersistenceStore.EncodeForDiagnostics(74, two, PersistenceStore.PreviousFormatVersion);
        store.WriteRawForDiagnostics("A", v1A);
        store.WriteRawForDiagnostics("B", v1B);
        PersistenceStore.RestoreResult v1Restore = store.Restore();
        ...
        Assert(PersistenceStore.DecodeForDiagnostics(store.ReadRawForDiagnostics("A")!).FormatVersion == 2 &&
        ...
        Assert(PersistenceStore.DecodeForDiagnostics(store.ReadRawForDiagnostics("A")!).FormatVersion == 2 &&
            PersistenceStore.DecodeForDiagnostics(store.ReadRawForDiagnostics("B")!).FormatVersion == 2,
        ...
        store.ClearNamespaceForDiagnostics();
        store.WriteRawForDiagnostics("A", v1A);
        byte[] corruptV2 = PersistenceStore.EncodeForDiagnostics(75, two);
        corruptV2[^1] ^= 1;
        ...
        store.ClearNamespaceForDiagnostics();
        store.WriteRawForDiagnostics("A", PersistenceStore.EncodeForDiagnostics(30, one));
        Assert(store.Restore().Generation == 30, "interrupted-migration-before-write-keeps-old");
        store.WriteRawForDiagnostics("B", PersistenceStore.EncodeForDiagnostics(31, two));
        Assert(store.Restore().Generation == 31, "interrupted-migration-after-complete-v2-selects-new");

        byte[] future = PersistenceStore.EncodeForDiagnostics(40, two);
        ...
        store.ClearNamespaceForDiagnostics();
        store.WriteRawForDiagnostics("A", PersistenceStore.EncodeForDiagnostics(50, empty, PersistenceStore.LegacyFormatVersion));
        PersistenceStore.RestoreResult v0 = store.Restore();
    ...
    private static void LimitBoundaries(PersistenceStore store, string sessionRoot, byte[] settings)
    {
        byte[] saveBelow = ExactSaveBytes(PersistenceStore.MaximumSaveDataBytes - 1);
        byte[] saveAt = ExactSaveBytes(PersistenceStore.MaximumSaveDataBytes);
        byte[] saveAbove = ExactSaveBytes(PersistenceStore.MaximumSaveDataBytes + 1);
        Assert(Category(PersistenceStore.EncodeForDiagnostics(60, Entries(settings, saveBelow, null, null))) == "none",
            "save-uncompressed-limit-minus-one");
        Assert(Category(PersistenceStore.EncodeForDiagnostics(61, Entries(settings, saveAt, null, null))) == "none",
            "save-uncompressed-limit-inclusive");
        Assert(Category(PersistenceStore.EncodeForDiagnostics(62, Entries(settings, saveAbove, null, null))) == "decompression-limit",
            "save-uncompressed-limit-plus-one");

        byte[] settingsBelow = ExactSettingsBytes(PersistenceStore.MaximumSettingsBytes - 1);
        byte[] settingsAt = ExactSettingsBytes(PersistenceStore.MaximumSettingsBytes);
        byte[] settingsAbove = ExactSettingsBytes(PersistenceStore.MaximumSettingsBytes + 1);
        Assert(Category(PersistenceStore.EncodeForDiagnostics(63, Entries(settingsBelow, null, null, null))) == "none",
            "settings-uncompressed-limit-minus-one");
        Assert(Category(PersistenceStore.EncodeForDiagnostics(64, Entries(settingsAt, null, null, null))) == "none",
            "settings-uncompressed-limit-inclusive");
        Assert(Category(PersistenceStore.EncodeForDiagnostics(65, Entries(settingsAbove, null, null, null))) == "decompression-limit",
            "settings-uncompressed-limit-plus-one");

        Assert(PersistenceStore.PolicyFailureCategoryForDiagnostics("compressed-entry",
            PersistenceStore.HardCompressedEntryBudgetBytes) == "none", "compressed-entry-limit-inclusive");
        Assert(PersistenceStore.PolicyFailureCategoryForDiagnostics("compressed-entry",
            PersistenceStore.HardCompressedEntryBudgetBytes + 1) == "compressed-entry-limit", "compressed-entry-limit-plus-one");
        Assert(PersistenceStore.PolicyFailureCategoryForDiagnostics("envelope",
            PersistenceStore.HardEnvelopeBudgetBytes) == "none", "envelope-limit-inclusive");
        Assert(PersistenceStore.PolicyFailureCategoryForDiagnostics("envelope",
            PersistenceStore.HardEnvelopeBudgetBytes + 1) == "envelope-limit", "envelope-limit-plus-one");
        Assert(PersistenceStore.PolicyFailureCategoryForDiagnostics("bridge-total",
            PersistenceStore.HardTotalBudgetBytes - 1, 1) == "none", "bridge-total-limit-inclusive");
        Assert(PersistenceStore.PolicyFailureCategoryForDiagnostics("bridge-total",
            PersistenceStore.HardTotalBudgetBytes, 1) == "bridge-total-limit", "bridge-total-limit-plus-one");

        store.ClearNamespaceForDiagnostics();
        store.WriteRawForDiagnostics("A", PersistenceStore.EncodeForDiagnostics(66, Entries(settings, null, null, null)));
    ...
    )
    {
        ...
        store.WriteRawForDiagnostics("A", PersistenceStore.EncodeForDiagnostics(80, one));
        ...
        store.WriteRawForDiagnostics("A", PersistenceStore.EncodeForDiagnostics(81, Entries(malformedSettings, null, null, null)));
        ...
        store.WriteRawForDiagnostics("A", PersistenceStore.EncodeForDiagnostics(82, Entries(settings, malformedSave, null, null)));
        Assert(!store.Restore().Materialized && store.Restore().SlotAStatus == "invalid-serializer-invalid",
            "malformed-save-rejected");

        byte[] duplicate = PersistenceStore.EncodeForDiagnostics(83, one);
        ...
        store.ClearNamespaceForDiagnostics();
        store.WriteRawForDiagnostics("A", PersistenceStore.EncodeForDiagnostics(84, empty));
    ...
    private static void SaveManagerMutationSuite(
        ...
        byte[] saveA,
        byte[] saveB,
        IReadOnlyList<PersistenceStore.DiagnosticEntry> initialEntries
    )
    {
        store.ClearNamespaceForDiagnostics();
        byte[] priorGeneration = PersistenceStore.EncodeForDiagnostics(200, initialEntries);
        store.WriteRawForDiagnostics("A", priorGeneration);
        Assert(store.Restore().Generation == 200, "save-manager-prior-generation-restored");
        SaveExportSnapshot before = store.CreateReadOnlySaveManagerSnapshot();

        SaveMutationResult replaced = store.MutateFromSaveManager(new SaveMutationCommand(
        ...
        Assert(store.ReadRawForDiagnostics("B") != null &&
            PersistenceStore.DecodeForDiagnostics(store.ReadRawForDiagnostics("B")!).Generation == 201,
        ...
        SaveMutationResult stale = store.MutateFromSaveManager(new SaveMutationCommand(
        ...
        SaveMutationResult malformed = store.MutateFromSaveManager(new SaveMutationCommand(
        ...
        byte[] oversized = ExactSaveBytes(PersistenceStore.MaximumSaveDataBytes + 1);
        SaveMutationResult tooLarge = store.MutateFromSaveManager(new SaveMutationCommand(
        ...
        SaveMutationResult deleted = store.MutateFromSaveManager(new SaveMutationCommand(
        ...
        SaveMutationResult deleteAgain = store.MutateFromSaveManager(new SaveMutationCommand(
        ...
        SaveMutationResult reset = store.MutateFromSaveManager(new SaveMutationCommand(
        ...
        SaveMutationResult settingsReplace = store.MutateFromSaveManager(new SaveMutationCommand(
        ...
        byte[] newest = store.ReadRawForDiagnostics("A") != null &&
            PersistenceStore.DecodeFailureCategoryForDiagnostics(store.ReadRawForDiagnostics("A")!) == "none" &&
            PersistenceStore.DecodeForDiagnostics(store.ReadRawForDiagnostics("A")!).Generation == store.Generation
        ...
        string newestSlot = store.ReadRawForDiagnostics("A") != null &&
            PersistenceStore.DecodeFailureCategoryForDiagnostics(store.ReadRawForDiagnostics("A")!) == "none" &&
            PersistenceStore.DecodeForDiagnostics(store.ReadRawForDiagnostics("A")!).Generation == store.Generation ? "A" : "B";
        newest[^1] ^= 1;
        store.WriteRawForDiagnostics(newestSlot, newest);
        PersistenceStore.RestoreResult fallback = store.Restore();
        ...
        store.ClearNamespaceForDiagnostics();
        store.WriteRawForDiagnostics("A", PersistenceStore.EncodeForDiagnostics(300, initialEntries));
        Assert(store.Restore().Generation == 300, "save-manager-concurrency-baseline");
        SaveExportSnapshot concurrentBefore = store.CreateReadOnlySaveManagerSnapshot();
        SaveMutationCommand first = new("1", saveA, concurrentBefore.Generation, concurrentBefore.LogicalHash);
        SaveMutationCommand second = new("2", saveB, concurrentBefore.Generation, concurrentBefore.LogicalHash);
        Task<SaveMutationResult>[] concurrent =
        {
    ...
    private static void LargeFixtureSuite()
    {
        ...
        foreach (FixtureConfig configuration in configurations)
        {
            ...
            Assert(payload.Length <= PersistenceStore.MaximumSaveDataBytes, $"{configuration.Name}-uncompressed-budget");
            byte[] stored = PersistenceStore.CompressForDiagnostics(payload);
            Assert(stored.Length <= PersistenceStore.HardCompressedEntryBudgetBytes, $"{configuration.Name}-stored-budget");
            ...
            RuntimeLog.Info(
                ...
                $"raw-budget-percent={Percent(payload.Length, PersistenceStore.MaximumSaveDataBytes)}; " +
                $"stored-budget-percent={Percent(stored.Length, PersistenceStore.HardCompressedEntryBudgetBytes)}"
            );
        }

        byte[] settings = SettingsBytes(9, 8);
        byte[] threeSlot = PersistenceStore.EncodeForDiagnostics(100, Entries(
            settings,
            values["near-complete-profile"],
            values["abc-sides-and-202-strawberries"],
            values["chapter-9"]
        ));
        PersistenceStore.DiagnosticDecoded decoded = PersistenceStore.DecodeForDiagnostics(threeSlot);
        Assert(decoded.Entries.Count(entry => entry.Present) == 4, "large-three-slot-generation-decodes");
        Assert(decoded.EnvelopeBytes <= PersistenceStore.HardEnvelopeBudgetBytes, "large-three-slot-envelope-headroom");
        Assert(decoded.EnvelopeBytes * 2 <= PersistenceStore.HardTotalBudgetBytes, "large-three-slot-two-generation-headroom");
        RuntimeLog.Info(
            ...
            $"envelope-budget-percent={Percent(decoded.EnvelopeBytes, PersistenceStore.HardEnvelopeBudgetBytes)}; " +
            $"bridge-budget-percent={Percent(decoded.EnvelopeBytes * 2, PersistenceStore.HardTotalBudgetBytes)}"
        );
    }
    ...
    private static void RetainedPhysicalFixtureSuite()
    {
        ...
        foreach ((string name, (int bytes, string hash)) in expected)
        {
            ...
            byte[] encoded = PersistenceStore.EncodeForDiagnostics(110, Entries(SettingsBytes(10, 10), payload, null, null));
            PersistenceStore.DiagnosticDecoded decoded = PersistenceStore.DecodeForDiagnostics(encoded);
            Assert(decoded.Entries.Single(entry => entry.Name == "0").UncompressedBytes == bytes, $"retained-{name}-v2-roundtrip");
            RuntimeLog.Info($"STAGE9B_RETAINED_FIXTURE name={name}; raw-bytes={bytes}; compressed-bytes={PersistenceStore.CompressForDiagnostics(payload).Length}; sha256={hash}");
        }
    }
#endif

    private static IReadOnlyList<PersistenceStore.DiagnosticEntry> Entries(byte[]? settings, byte[]? slot0, byte[]? slot1, byte[]? slot2) =>
        new[]
        {
            Entry("settings", settings, PersistenceStore.SerializerKind.Settings),
            Entry("0", slot0, PersistenceStore.SerializerKind.SaveData),
            Entry("1", slot1, PersistenceStore.SerializerKind.SaveData),
            Entry("2", slot2, PersistenceStore.SerializerKind.SaveData)
        };

    private static PersistenceStore.DiagnosticEntry Entry(
        string name,
        byte[]? payload,
        PersistenceStore.SerializerKind serializer
    ...
    private static void WriteSessionFile(string sessionRoot, string logicalName, byte[] payload)
    {
        ...
        PersistenceStore.WriteMaterializedPayload(path, payload);
    }

    private static string Category(byte[] encoded) => PersistenceStore.DecodeFailureCategoryForDiagnostics(encoded);

    private static List<V2EntryLayout> Layouts(byte[] value)
    {
        if (BinaryPrimitives.ReadUInt16LittleEndian(value.AsSpan(8)) != PersistenceStore.FormatVersion)
    ...
    private static void Assert(bool condition, string name)
    {
        ...
        passed++;
        RuntimeLog.Info($"STAGE6_TEST name={name}; result=PASS");
    }
