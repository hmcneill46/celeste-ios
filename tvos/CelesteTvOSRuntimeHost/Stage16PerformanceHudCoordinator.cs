...
internal sealed class Stage16PerformanceHudCoordinator : IDisposable
{
    ...
    private readonly Stage16PerformanceHudPreferenceState preference = new(new UserDefaultsStore());
    ...
    private Stage16PerformanceHudMode? appliedMode;
    private bool available;
    private bool disposed;

    internal Stage16PerformanceHudCoordinator()
    {
        ...
        Stage3BLog.Info($"STAGE16B_HUD coordinator-ready=true; requested={preference.Mode}; key-schema=v1; logging=disabled");
    }
    ...
    private void ApplyBeforeFirstRender()
    {
        lock (gate)
        {
            ...
            Stage3BLog.Info($"STAGE16B_HUD before-first-render=true; layer-replaced={replaced.ToString().ToLowerInvariant()}; candidates={candidates}; draws={TvOSStage3Bridge.DrawCount}");
        }
    }
    ...
    private void HideForInactiveLifecycle()
    {
        lock (gate)
        {
            ...
            Stage16PerformanceHudMode hidden =
                Stage16PerformanceHudPolicy.ModeWhileInactive(preference.Mode);
            Apply(hidden, "resign-active");
            Stage3BLog.Info($"STAGE16B_HUD lifecycle=resign-active; requested={preference.Mode}; visible=Off; logging=disabled");
        }
    }

    private bool IsEnabled()
    {
        lock (gate) return available && preference.Mode == Stage16PerformanceHudMode.On;
    }
    ...
    private void ApplyAtStartup(IntPtr window)
    {
        lock (gate)
        {
            ...
            Stage3BLog.Info($"STAGE16B_HUD real-presentation-layer=true; candidates={candidates}; drawable-match=true; device=present; first-draw=false");
        }
    }
    ...
    private void SetEnabled(bool enabled)
    {
        ...
        {
            if (disposed) return;
            Stage16PerformanceHudMode requested = enabled ? Stage16PerformanceHudMode.On : Stage16PerformanceHudMode.Off;
            ...
            Stage3BLog.Info($"STAGE16B_HUD requested={preference.Mode}; persisted={(changed ? "changed" : "unchanged")}; live=true; logging=disabled; draws={TvOSStage3Bridge.DrawCount}");
        }
    }
    ...
    private void RevalidateOnForeground()
    {
        lock (gate)
        {
            ...
            available = true;
            if (replaced) appliedMode = null;
            Stage16PerformanceHudMode active =
                Stage16PerformanceHudPolicy.ModeAfterForeground(preference.Mode);
            Apply(active, replaced ? "foreground-replacement" : "foreground-stable");
            Stage3BLog.Info($"STAGE16B_HUD foreground=true; layer-replaced={replaced.ToString().ToLowerInvariant()}; candidates={candidates}");
        }
    }

    private bool Apply(Stage16PerformanceHudMode mode, string reason)
    {
        ...
        try
        {
            presentationLayer.DeveloperHudProperties =
                mode == Stage16PerformanceHudMode.On ? OnProperties : OffProperties;
            appliedMode = mode;
            Stage16HudProperties properties = Stage16PerformanceHudPolicy.Properties(mode);
            Stage3BLog.Info($"STAGE16B_HUD apply=true; mode={properties.Mode}; logging={properties.Logging}; reason={reason}; draws={TvOSStage3Bridge.DrawCount}");
            return true;
        }
        catch (Exception exception)
        {
            Stage3BLog.Warning($"STAGE16B_HUD apply=false; category={exception.GetType().Name}; reason={reason}; gameplay=continuing");
            available = false;
            appliedMode = null;
            return false;
        }
    ...
    private void FailClosed(string category, int candidateCount)
    {
        ...
        available = false;
        appliedMode = null;
        Stage3BLog.Warning($"STAGE16B_HUD available=false; category={category}; candidates={candidateCount}; gameplay=continuing");
    }
        ...
        out int candidateCount)
    {
        ...
        Stage16MetalLayerCandidate[] candidates = metalIdentities
            .Select(identity => new Stage16MetalLayerCandidate(
                identity,
            ...
            .ToArray();
        IntPtr? selectedIdentity = Stage16PerformanceHudPolicy.SelectPresentationLayer(candidates);
    ...
    private sealed class UserDefaultsStore : IStage16PerformanceHudPreferenceStore
    {
        private readonly NSUserDefaults defaults = NSUserDefaults.StandardUserDefaults;

        public string? Read() => defaults.StringForKey(Stage16PerformanceHudPolicy.PreferenceKey);

        public void Write(string value)
        {
            defaults.SetString(value, Stage16PerformanceHudPolicy.PreferenceKey);
