...
internal sealed class SoftReloadCoordinator : IDisposable
{
    ...
    private readonly SoftReloadStateMachine stateMachine = new();
    private PersistenceStore.ExternalMutationReloadTicket? ticket;
    ...
    internal SoftReloadCoordinator(PersistenceStore store, SaveManagerService manager)
    {
    ...
    private bool RequestReload()
    {
        lock (gate)
        {
            ...
            {
                RuntimeLog.Warning("STAGE13B_REQUEST result=rejected; reason=no-verified-mutation-or-disposed");
                return false;
            }
            if (!stateMachine.TryRequest(verifiedExternalMutation: true))
            {
                RuntimeLog.Info($"STAGE13B_REQUEST result=duplicate-suppressed; state={PhaseName(stateMachine.State)}");
                return false;
            }
    ...
    {
        PersistenceStore.ExternalMutationReloadTicket current = ticket
    ...
    private void ValidateSettings(PersistenceStore.ExternalMutationReloadTicket current)
    {
    ...
    private void ValidateSave(string logicalName, PersistenceStore.ExternalMutationReloadTicket current)
    {
