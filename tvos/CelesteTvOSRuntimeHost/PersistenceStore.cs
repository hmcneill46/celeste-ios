...
internal sealed class PersistenceStore : IDisposable
{
    ...
    internal PersistenceStore(string sessionRoot, string storageNamespace)
    {
        ...
        sizeObserver = NSNotificationCenter.DefaultCenter.AddObserver(
            ...
            _ =>
            {
                ...
                RuntimeLog.Error("STAGE6_DEFAULTS_SIZE_LIMIT_WARNING category=userdefaults-size-warning; observed=true; acceptance=failure");
            });
    ...
    internal RestoreResult Restore()
    {
        lock (gate)
        {
            ...
            if (selected == null)
            {
                RuntimeLog.Warning($"STAGE6_RESTORE namespace={NamespaceCategory}; selected=none; A={a.Status}; B={b.Status}; action=new-game-no-write");
    ...
    internal bool Commit(string reason)
    {
        lock (gate)
        {
            try
            {
                ...
                {
                    RuntimeLog.Info($"STAGE6_COMMIT reason={SanitizeReason(reason)}; result=unchanged-new-game; generation=0; logical=none");
                    return true;
                }
    ...
    internal SaveExportSnapshot CreateReadOnlySaveManagerSnapshot()
    {
        lock (gate)
        {
            ThrowIfDisposed();
    ...
    internal SaveMutationResult MutateFromSaveManager(SaveMutationCommand command)
    {
        lock (gate)
        {
            ...
            try
            {
                        ...
                        Encoding.ASCII.GetBytes(currentHash)))
                {
                    ...
                    return SaveMutationResult.ConflictResult(CreateExportSnapshotUnsafe());
                }
                ...
                if (!changed)
                {
                    ...
                    return SaveMutationResult.Unchanged(CreateExportSnapshotUnsafe(), externalMutationRequiresRestart);
                }
                ...
                SaveExportSnapshot exported = CreateExportSnapshotUnsafe();
                RuntimeLog.Info(
                ...
                return SaveMutationResult.Committed(exported, encoded.Length, BridgeBytes());
            }
            catch (Exception exception)
            {
                LastFailureCategory = Category(exception);
                ...
                return SaveMutationResult.Failed(LastFailureCategory, CreateExportSnapshotUnsafe(), externalMutationRequiresRestart);
            }
        }
    }

    // Soft reload keeps the external-mutation guard set while it rebuilds only
    ...
    private SaveExportSnapshot CreateExportSnapshotUnsafe()
    {
        Dictionary<string, byte[]?> files = new(StringComparer.Ordinal);
        foreach (string logicalName in new[] { "settings", "0", "1", "2" })
        {
        ...
        return new SaveExportSnapshot(selected?.Generation ?? 0, selected?.LogicalHash ?? "none", files);
    }
    ...
    private static string PrefixForNamespace(string value) => value switch
    {
        ...
        _ => throw new InvalidOperationException("Persistence storage namespace must be a committed fixed category.")
    };
    ...
    private void ThrowIfDisposed() { if (disposed) throw new ObjectDisposedException(nameof(PersistenceStore)); }
