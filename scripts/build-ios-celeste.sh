...
Build the experimental modern-iOS Celeste product for a physical arm64 iPhone.
...
for required in \
  ...
  "$REPO_ROOT/artifacts/ios-celeste/current/ios-managed.json"; do
...
python3 - "$OUTPUT_DIR/build-manifest.json" "$OUTPUT_DIR/Celeste-modern-iOS.ipa" "$elapsed" \
  ...
  "$REPO_ROOT/artifacts/ios-celeste/current/ios-managed.json" <<'PY'
