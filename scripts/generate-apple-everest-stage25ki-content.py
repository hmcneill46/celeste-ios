#!/usr/bin/env python3
"""Regenerate the bounded K-I file plan directly from the pinned public ZIPs."""
...
def generate(packages_root):
    ...
    return {"schemaVersion": 1, "id": "stage25ki-sj-beginner-slice-v1", "packages": records}
...
def main():
    ...
    parser.add_argument("--compare-kg", action="store_true")
    args = parser.parse_args()
    value = generate(args.packages_root)
    if args.compare_kg:
        historical = json.loads(subprocess.check_output(
            ["git", "-C", str(ROOT), "show", KG + ":apple-everest/sj-beginner-content-stage25kg.json"], text=True))
        if value["packages"] != historical["packages"]:
            raise ValueError("regenerated plan differs from immutable K-G evidence")
    ...
    if args.compare_kg:
        print("PASS: all selected paths, archive/logical identities and file hashes match immutable K-G")
