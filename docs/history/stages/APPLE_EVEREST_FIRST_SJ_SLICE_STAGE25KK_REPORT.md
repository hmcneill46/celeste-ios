...
The first disposable clone exposed a host-verifier output-path defect: Git cannot check whether an absolute path outside its checkout is tracked. The correction limits that Git query to paths within the checkout, preserving the existing tracked-file protection while allowing external receipts. Final verification uses a new recursive clone of the corrected commit, with fresh public acquisition. This host-only correction does not change the terrain evidence or any product/runtime source.
