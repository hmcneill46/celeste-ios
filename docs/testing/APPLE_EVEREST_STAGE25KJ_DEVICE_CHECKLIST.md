...
This build tests project-owned factory fixtures. It contains no original Strawberry Jam gameplay map. Build 40 is the replacement K-J candidate; acceptance below remains pending until tested on the exact signed products. Build 38 stopped at MaxMechanics/factory_04 because the return berry's profile guard rejected checkpoint/order values assigned by the map loader. Build 37 failed at the masked star background and exposed map-sprite and cat-placement defects. Build 39 passed the 73/73 sweep on iPhone 12, but exposed dialogue-scope and Collab presentation/input differences; Apple TV was not tested. Earlier builds have no transferable build 40 physical acceptance.

## Start here

1. Open **Celeste Everest Canary** and confirm the Options version is **0.1.1 (40)**.
