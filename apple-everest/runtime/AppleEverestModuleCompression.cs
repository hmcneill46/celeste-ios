...
// tvOS keeps module durability in a separate bounded UserDefaults namespace.
// This envelope mirrors the project's proven compressed-save design without
// changing the platform-neutral logical snapshot or its hashes.
internal static class AppleEverestModuleCompression
{
    private static readonly byte[] Magic = Encoding.ASCII.GetBytes("AEVMZV1\0");
    internal const int MaximumLogicalBytes = 512 * 1024;
    ...
    internal static byte[] Encode(byte[] logical)
    {
        if (logical == null || logical.Length == 0 || logical.Length > MaximumLogicalBytes)
            throw new InvalidDataException("Apple Everest tvOS module state exceeds its logical budget");
        ...
        using (BinaryWriter writer = new(envelope, new UTF8Encoding(false, true), leaveOpen: true))
        {
            writer.Write(Magic);
            writer.Write(1);
            writer.Write(logical.Length);
            writer.Write(SHA256.HashData(logical));
        ...
        if (unsigned.Length + digest.Length > MaximumReplicaBytes)
            throw new InvalidDataException("Apple Everest tvOS module state exceeds its compressed replica budget");
    ...
    internal static bool TryDecode(byte[] envelope, out byte[] logical)
    {
        ...
        if (envelope == null || envelope.Length < Magic.Length + 4 + 4 + HashBytes + 4 + HashBytes ||
            envelope.Length > MaximumReplicaBytes)
            return false;
        ...
        if (!CryptographicOperations.FixedTimeEquals(SHA256.HashData(envelope.AsSpan(0, unsignedLength)),
                envelope.AsSpan(unsignedLength, HashBytes)))
            return false;
        try
        {
            ...
            if (!reader.ReadBytes(Magic.Length).SequenceEqual(Magic) || reader.ReadInt32() != 1)
                return false;
            int logicalLength = reader.ReadInt32();
            byte[] expectedHash = reader.ReadBytes(HashBytes);
            int compressedLength = reader.ReadInt32();
            if (logicalLength is < 1 or > MaximumLogicalBytes || expectedHash.Length != HashBytes ||
                compressedLength < 1 || compressedLength != input.Length - input.Position)
                return false;
            using DeflateStream deflate = new(input, CompressionMode.Decompress, leaveOpen: false);
            byte[] result = new byte[logicalLength];
            int offset = 0;
            ...
            if (deflate.ReadByte() != -1 ||
                !CryptographicOperations.FixedTimeEquals(expectedHash, SHA256.HashData(result)))
                return false;
            logical = result;
            return true;
        }
