...
// Queue and constructor are emitted by the selected semantic pool registry.
internal sealed class AppleEverestVivCrystalDebris : Actor
{
	...
	public AppleEverestVivCrystalDebris()
		: base(Vector2.Zero)
	{
		base.Depth = -9990;
	...
	public static void Burst(Vector2 position, Color color, bool boss, int count = 1, string imagePath = "particles/shard", float scale = 1f)
	{
		...
		{
			AppleEverestVivCrystalDebris customCrystalDebris = Engine.Pooler.Create<AppleEverestVivCrystalDebris>();
			Vector2 position2 = position + new Vector2(Calc.Random.Range(-4, 4), Calc.Random.Range(-4, 4));
			customCrystalDebris.Init(position2, color, boss, new Image(GFX.Game[imagePath]), scale);
			Engine.Scene.Add(customCrystalDebris);
		}
	}
